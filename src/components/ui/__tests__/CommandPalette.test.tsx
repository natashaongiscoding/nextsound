...
describe('CommandPalette', () => {
  ...
  describe('Keyboard Navigation', () => {
    ...
    it('should handle enter key to select item', async () => {
      ...
      await waitFor(() => {
        expect(mockAudioPlayer.playTrack).toHaveBeenCalledWith(
          expect.objectContaining({ name: 'Song 1' })
        );
        expect(onClose).toHaveBeenCalled();
      });
    });
  ...
  describe('Item Selection', () => {
    it('should play track when track item is clicked', async () => {
      const user = userEvent.setup();
      const onClose = vi.fn();
      const mockSearchResponse = {
        data: {
      ...
      expect(mockAudioPlayer.playTrack).toHaveBeenCalledWith(
        expect.objectContaining({ name: 'Clickable Song' })
      );
      expect(onClose).toHaveBeenCalled();
    });
